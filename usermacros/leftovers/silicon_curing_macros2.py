...
def initialize_posdict():
    posdict={
        'slot1':{'md':{'sample':'0.17% DOTL new','fill_time':1774640207.7744472},
        'positions':{
        '1':{'y_start':-3.2,'h':-3.5,'x_start':-3.0,'y':-3.2,'x':-3.0},
        '2':{'y_start':-2.75,'h':-3.0,'x_start':-3.0,'y':-2.75,'x':-3.0},
        '3':{'y_start':-2.,'h':-1.45,'x_start':-3.0,'y':-2.,'x':-3.0},
        }
        }
...
def set_filltime(posdict,slot):
    ## Works for single slot
    current_time = time.time()
    posdict[slot]['md']['fill_time'] = current_time
...
def initial_sampling(slot): ## this covers the first ~120 minutes of curing
    ...
    for i in range(1):
        for h in ['1','2','3']:
            ...
            att2.set_T(.0013*5)
            series(det='eiger4m',expt=0.2,imnum=200, feedback_on=True,auto_compression=True,analysis='phi_4x_20deg',OAV_mode='none',comment='AUTO_COMMENT')
...
def regular_sampling(slot): ## this covers ~60 minutes to ~120 minutes of curing
    ...
    for i in range(1):
        for h in ['1','2','3']:
            ...
            # att2.set_T(.0013/5/5)
            # goto_posdict(RE.md['posdict'],slot,h)
            # series(det='eiger4m',expt=12.5,imnum=200, feedback_on=True,auto_compression=True,analysis='phi_4x_20deg',OAV_mode='none',comment='AUTO_COMMENT')
            # temp_posdict=posdict_nextpos(RE.md['posdict'],slot,h,stepsize=.15);RE.md['posdict']=temp_posdict  
        
def regular_sampling_no_h1(slot): ## this covers ~60 minutes to ~120 minutes of curing
    RE.md['sample']=RE.md['posdict'][slot]['md']['sample']
    RE.md['fill_time']=RE.md['posdict'][slot]['md']['fill_time']
    for i in range(1):
        for h in ['2','3']:
...
def initial_curing_measurements(slots): ## this covers the full first 16 hours of curing (~1000 min). Does all heights in one slot before switching to the next slot
    # RE.md['posdict'] = posdict ## run initialize_posdict() first, then run set_filltime() when you fill sample
    ## slots must be list of strings: ['slot1','slot2']
    for i in range(6):
        for s in slots:
            initial_sampling(s)
...
def late_curing_measurements(slots): ## this covers the full first 16 hours of curing (~1000 min). Does all heights in one slot before switching to the next slot
    ## slots must be list of strings: ['slot1','slot2']
    for k in range(6):
        for s in slots:
            intermediate_sampling(s)
...
#### restart humidity ######################
# from either Bluesky session: set_flows(dry=2.4,wet=2.4)
# in BlueSky2: rh_control(rh_target=50)
